...
from pycoinnet.examples.blockarchive import BlockArchive
...
def block_processor(archive, change_q, blockfetcher, state_dir):
    MAX_BLOCKS_CHANGESET = 100
    block_q = collections.deque()
    last_processed_block = get_last_processed_block(state_dir)
    while True:
        # if we're up here, either block_q is empty, or change_q has stuff currently available
        add_remove, block_hash, block_index = yield from change_q.get()
        # are we adding a block?
        if add_remove == "add":
            # yes, we're adding a block
            # if we are too early in the blockchain to care, don't do anything
            if block_index < last_processed_block:
                continue
            item = (blockfetcher.get_block_future(block_hash, block_index), block_hash, block_index)
            block_q.append(item)
            if len(block_q) < MAX_BLOCKS_CHANGESET and change_q.qsize() > 0:
                continue
        elif add_remove == "remove":
            # look at block_q right side and remove it if appropriate
            if len(block_q) > 0:
                q_block_future, q_block_hash, q_block_index = block_q[-1]
                if block_index == q_block_index:
                    logging.debug("popping block index %d from block_q", q_block_index)
                    block_q.pop()
            continue
        else:
        ...
        # either change_q has nothing available or len(block_q) > MAX_BLOCKS_CHANGESET

        # build a reorg object
        base_block_index = block_q[0][-1]
        blocks = []
        block_future, block_hash, block_index = block_q.popleft()
        block = yield from block_future
        blocks.append(block)
        while len(block_q) > 0 and block_q[0][0].done():
            block_future, block_hash, block_index = block_q.popleft()
            block = yield from block_future
            blocks.append(block)
        archive.create_block_update(blocks, base_block_index)
        update_last_processed_block(state_dir, base_block_index+len(blocks))
...
def main():
    ...
    archive = BlockArchive(args.blockdir)
    block_processor_task = asyncio.Task(block_processor(archive, change_q, blockfetcher, state_dir))
